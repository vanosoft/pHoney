...
from parser.exceptions import GrammarError, ParserInitError
from parser.actions import pass_single, pass_none, collect, collect_sep
from parser.common import Location, load_python_module
from parser.trees import visitor
from parser.termui import prints, s_emph, s_header, a_print, h_print
from parser import termui
...
class PGFile(object):
    """Objects of this class represent parser grammar files.
    ...
    classes (dict of parserClass): Dynamically created classes. Used by
        obj action.
    ...
    def make_multiplicity_symbol(self, symbol_ref, base_symbol, separator,
        ...
        if mult in [MULT_ONE_OR_MORE, MULT_ZERO_OR_MORE]:
            ...
            symbol = self.resolve_symbol_by_name(symbol_name)
            if not symbol:
                # noqa See: http://www.igordejanovic.net/parser/grammar_language/#one-or-more_1
                productions = []
...
class Grammar(PGFile):
    ...
    def _resolve_actions(self, action_overrides=None,
        ...
        """
        import parser.actions as actmodule
            ...
            if symbol.action_name and action is None \
               and fail_on_no_resolve:
                raise ParserInitError(
                    ...
                    'doesn\'t exists in parser common actions and '
...
def _create_prods(context, rhs_prods, name, rule_meta_datas):
    ...
    if attrs:
        class parserClass(object, metaclass=parserMetaClass):
            """Dynamically created class. Each parser rule that uses named
        ...
        parserClass.__name__ = str(symbol.fqn)
        if symbol.fqn in context.extra.classes:
            ...
            context.extra.classes[symbol.fqn] = parserClass
...
class parserMetaClass(type):
    ...
    def __repr__(cls):
        return '<parser:{} class at {}>'.format(cls.__name__, id(cls))
